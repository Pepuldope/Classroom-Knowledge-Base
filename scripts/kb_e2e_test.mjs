...
test("kbDensityClass maps the local reading-density setting to a view class", () => {
  ...
  assert.equal(kbDensityClass({ density: "comfortable" }), "kb-density-comfortable");
  assert.equal(kbDensityClass({ density: "invalid" }), "kb-density-comfortable");
});
...
test("related preview error model exposes a scoped keyboard retry action", () => {
  assert.deepEqual(relatedPreviewRetryModel(), {
    label: "Retry related notes",
    ariaLabel: "Retry loading related notes",
    focusable: true,
  });
});

test("repeated related-preview failures announce the retry attempt distinctly", () => {
  assert.deepEqual(relatedPreviewErrorModel(1), {
    message: "Related notes unavailable",
    announcement: "Related notes unavailable. Retry loading related notes.",
  });
  assert.deepEqual(relatedPreviewErrorModel(2), {
    message: "Related notes still unavailable",
    announcement: "Related notes still unavailable after 2 attempts. Retry loading related notes.",
  });
});
...
// ---------------------------------------------------------------------------
test("resetTutorConversation clears the current thread without mutating the input", () => {
...
test("related preview timing percentiles summarize bounded warm samples", () => {
  assert.deepEqual(
    relatedPreviewTimingPercentiles([12.5, 4, "bad", -1, 8, 20], { limit: 4 }),
    { samples: 4, p50Ms: 8, p95Ms: 20 },
  );
  assert.deepEqual(relatedPreviewTimingPercentiles([], { limit: 4 }), { samples: 0, p50Ms: 0, p95Ms: 0 });
});
...
test("development harness exposes a bounded related-cache reset control", async () => {
  ...
  assert.match(source, /id="kbRelatedTimingPercentiles"/);
