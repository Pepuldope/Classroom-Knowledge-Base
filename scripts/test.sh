...
"kb|srv|Tutor stall: an answer that never comes says so and offers Retry|node scripts/tutor_stall_test.mjs"
...
"models,kb|-|Per-class export model tests (selection, file tree, zip writer)|node --test tests/kb-export.test.js"
"models,kb|-|ZIP writer round-trip through a real unzip|node scripts/zip_roundtrip_test.mjs"
...
"kb|seed|Manage e2e: rebuild reports where it was clicked, per-class export + attachments|node scripts/manage_export_test.mjs"
"kb,mobile|seed|Class-category board: touch drag, menu, reset|node scripts/class_board_test.mjs"
...
"settings|seed|Settings: your own AI key reaches the tutor and nothing else|node scripts/byok_settings_test.mjs"
