...
- [ ] Visual common-sense gate: extend the contrast browser gate to cover interactive states, overflow/clipping, overlaps, and KB/Archive/Planner/Settings surfaces. **Root cause:** existing checks are geometry + light-theme screenshots only (`theme_test.mjs` = string normalize; settings styling = one select not transparent; kb_ui “visual gate” = radius/cursor/centering, explicitly no screenshot analysis; LOOP-GUARDRAILS only bans blank/error shots). Live repro: dark `.step-text` ≈ **1.05:1** contrast on near-white card. **Ship one deterministic browser gate** (extend `kb_ui_test.mjs` / new `scripts/visual_common_sense_test.mjs`, wire into `scripts/test.sh`) that on **light AND dark** (and system+emulated prefers-color-scheme if cheap) does ALL of: (1) **Contrast** — sample computed fg/bg on representative text (body, muted/meta, chips, cards, onboarding `.step-text`, settings labels, buttons, inputs, status); fail if normal text < 4.5:1 or large/UI text < 3:1. (2) **Button / control interactive states** — for primary, secondary, nav/view-toggle, chips, icon buttons, and Settings controls, assert **default, :hover, :active (pressed), :focus-visible, and :disabled** (where applicable) have visible stylistic differentiation (background and/or border and/or color change vs default; pressed ≠ hover-only clone if both exist; focus ring non-zero outline/box-shadow; disabled reduced emphasis and not-identical to default). Use Playwright `hover` / `focus` / CDP or forced class + `getComputedStyle` — not screenshots alone. (3) **Common-sense layout hygiene** — fail on: text overflowing its box (`scrollWidth/Height` meaningfully > client, or `getClientRects` past parent padding box); ellipsis without accessible name if truncated; horizontal page overflow (`documentElement.scrollWidth > clientWidth+2`); controls/labels clipped by `overflow:hidden` parents; obvious stacked overlaps of interactive targets; zero-size clickable controls that still show text. (4) Cover KB + Archive + Planner + Settings + onboarding/empty/loading + at least one modal. Status may NOT say “screenshot verified” / “Settings styling passed” unless this gate ran green. Fix all failing surfaces found. **After shipping, keep the gate in `scripts/test.sh` as a standing “is the feature working?” layer** — run it on every UI-affecting feature alongside unit/API e2e and interaction e2e; screenshots stay supporting evidence only.
