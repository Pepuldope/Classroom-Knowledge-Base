...
- [ ] Theme/contrast: after a recent style change, some text is white-on-white (or otherwise unreadable). **Root cause of missed detection (2026-07-25):** existing “visual check” does NOT measure contrast. `theme_test.mjs` only unit-tests theme string normalization; `settings_ui_test.mjs` only checks one select’s radius/padding/non-transparent bg; `kb_ui_test.mjs` “vision-independent visual gate” checks border-radius, pointer cursor, and horizontal centering — explicitly “no screenshot analysis”; LOOP-GUARDRAILS screenshot rule only requires “results + chips, not blank/error.” Cron screenshots are captured in **default light** theme only. Live repro: dark theme onboarding `.step-text` is `color: rgb(243,244,246)` on `background: rgb(249,250,251)` → WCAG contrast **~1.05:1** (unreadable). Fix all light/dark/system surfaces (KB, Archive, Planner, Settings, modals, empty/loading, cards, onboarding steps). **Hard gate required:** browser test that forces `data-theme=dark` and `light`, samples computed fg/bg on representative text nodes, and **fails if contrast < 4.5:1** (or < 3:1 for large text). Screenshots alone / “Settings styling: passed” do NOT count.
